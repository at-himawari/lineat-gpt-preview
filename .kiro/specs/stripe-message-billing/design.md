...
1. **メッセージ枠拡張**: プレミアムユーザーが 1 日で 100 件のメッセージ制限を超えた場合、Stripe 決済を通じて追加の枠を購入できる
2. **プレミアムモデルアップグレード**: より高度な AI モデル（Gemini Pro）へのアクセスを購入できる

**重要な変更**: プレミアムプランのユーザーは 1 日 100 件、非プレミアムユーザーは 1 日 30 件のメッセージ制限を持ちます。
...
3. データベースでユーザーを作成/更新
4. ユーザーのモデルサブスクリプション状態を確認
5. メッセージ枠をチェック
   - プレミアムユーザー: 100 件未満 → 処理を続行、100 件到達 → Stripe 決済リンクを送信して終了
   - 非プレミアムユーザー: 30 件未満 → 処理を続行、30 件到達 → Stripe 決済リンクを送信して終了
6. 適切な AI モデル（Basic または Premium）を選択
7. 適切な AI モデル（Basic または Premium）を選択
8. Gemini API で応答を生成
9. 応答を LINE で返信
10. メッセージと応答をデータベースに保存
...
```javascript
// checkAndUpdateMessageLimit: プレミアムユーザーは100件、非プレミアムユーザーは30件制限を適用
async function checkAndUpdateMessageLimit(userId, isPremium)
```
...
```sql
ALTER TABLE users
MODIFY COLUMN message_count_3days INT DEFAULT 0,  -- プレミアムユーザーの使用カウント（0から開始）
ADD COLUMN has_premium_model BOOLEAN DEFAULT FALSE,
ADD COLUMN premium_activated_at TIMESTAMP NULL;

-- 注: message_count_3days は「残り枠」ではなく「使用カウント」として使用
-- プレミアムユーザー: 0から開始し、メッセージ送信ごとに+1、100で上限
-- 非プレミアムユーザー: 0から開始し、メッセージ送信ごとに+1、30で上限
```
...
*任意の*プレミアムユーザーに対して、メッセージを送信するとメッセージカウントが正確に 1 増加する
**検証: 要件 1.2**

### プロパティ 2: 1 日経過後のカウントリセット

*任意の*プレミアムユーザーに対して、カウント期間開始から 1 日以上経過した場合、次のメッセージ送信時にカウントが 0 にリセットされる
**検証: 要件 1.3**

### プロパティ 3: プレミアムユーザーの枠内メッセージ処理

*任意の*プレミアムユーザーに対して、メッセージカウントが 100 未満の場合、メッセージを送信するとシステムは応答を生成する
**検証: 要件 1.4**

### プロパティ 3.1: 非プレミアムユーザーの 30 件制限メッセージ処理

*任意の*非プレミアムユーザーに対して、メッセージカウントが 30 未満の場合、メッセージを送信するとシステムは応答を生成する
**検証: 要件 1.6**

### プロパティ 4: プレミアムユーザーの枠超過時の決済リンク送信

*任意の*プレミアムユーザーに対して、メッセージカウントが 100 に達した状態でメッセージを送信すると、Stripe 決済リンクを含む応答が返される
**検証: 要件 2.1**

### プロパティ 5: 決済完了後の枠追加

*任意の*プレミアムユーザーに対して、quota_extension 決済が完了すると、追加メッセージ枠が付与される
**検証: 要件 2.4**
...
*任意の*プレミアムユーザーと任意の購入回数に対して、枠期間内の複数の quota_extension 購入はすべて累積される
**検証: 要件 8.3**

### プロパティ 13: 枠拡張時のリセット時刻不変性

*任意の*プレミアムユーザーに対して、枠拡張を購入しても枠期間のリセット時刻は変更されない
**検証: 要件 8.5**
