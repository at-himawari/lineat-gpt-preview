...
1. **メッセージ枠管理**
   ...
   - すべてのユーザーに 1 日で 30 件のメッセージ枠を提供
   - メッセージ送信ごとに枠を 1 減算
   - 1 日経過後に自動的に 30 件にリセット
   - 枠がゼロになると決済リンクを送信
...
3. **商品と価格の作成**: Dashboard → Products
   - メッセージ枠拡張商品を作成（例: 30 件 / 300 円）
   - プレミアムモデル商品を作成（例: 1,400 円）
   - 各商品の Price ID をコピー
...
```
Bot: 申し訳ございません。1日で30通のメッセージ制限に達しました。

追加で30件のメッセージ枠を購入いただけます。
以下のリンクから決済を完了してください：
https://checkout.stripe.com/...
```
